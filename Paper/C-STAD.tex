...
%and we forecast their future values for all cohorts in $c_3$ using the best fitting univariate ARIMA($p$,$d$,$q$) model. The orders of the ARIMA model are chosen via the automatic algorithm available in the \texttt{R} package \texttt{forecast}, which is based on the minimization of the corrected Akaike Information Criterion \citep{hyndman2008automatic,hyndman2018forecastR}.
...
%For the second group of cohorts, we employ the same estimation procedure, with only a small modification. On one hand, since the modal age at death has been observed, there are no problems in the estimation of the parameter $s_c$. On the other hand, $D_{x,c}$ and $E_{x,c}$ are not observed at all ages; Eq.~(\ref{Eq:loglike}) can nonetheless be computed by giving zero weights to the unobserved data. This can be achieved by replacing zeros to the elements of $\bm{w}$ corresponding to the missing data.  \par

%Finally, for the third group of partially observed cohorts in $c_3$, we forecast the C-STAD parameters using the estimates of the first two groups. The lack of knowledge about the modal age at death indeed makes it impossible to estimate the parameter $s_c$ from the partially observed data, compute the log-likelihood function in Eq.~(\ref{Eq:loglike}) and estimate the remaining parameters. As such, we specify two different multivariate vector autoregressive time-series models for the C-STAD parameters. The first model is fitted to the parameters $s$ and $b_U$. From a theoretical perspective, these parameters are related by the fact that mortality changes occurring above the mode could modify its value \cite[see][Appendix B]{canudas2010three}. The second model is specified for $b_L$, $c_L$ and $d_L$: these three parameters are theoretically related to each other since they pertain to the same segment of the age-at-death distribution. The results for the countries analysed indeed confirm these hypotheses, showing high correlation between the two set of parameters and low correlation between the other combinations (see Fig.~\ref{Fig:correlation} in Appendix \ref{Appendix:AdditResults}).    
...
%and the logarithm of the unobserved age-specific mortality rates at age $x$ and cohort $c$ ($\ln(m_{x,c})$)
...
%Moreover, we compare the forecast unobserved $\ln(m_{x,c})$ (i.e.~ $\ln(m_{110+;1896,\ldots,1905})$, $\ln(m_{109;1897,\ldots,1905})$, $\ln(m_{108;1898,\ldots,1905}),\ldots$) with the true values.  \par
...
%For $\ln(m_{x,c})$, the RMSE is calculated as follows:
%\begin{equation}\label{Eq:RMSE_lmx}
%\mathrm{RMSE}=\frac{1}{\delta}\sum_{c=1}^{\delta}\sqrt{\frac{\sum_{x\in\chi} \left(\ln(\hat{m}_{x,c})-\ln (m_{x,c})\right)^2}{\mathrm{dim}(\chi)}}
%\end{equation} 
%where $\ln(\hat{m}_{x,c})$ and $\ln (m_{x,c})$ are the forecast and observed rates, and $\chi$ denotes the set of unobserved rates, whose length is equal to $\mathrm{dim}(\chi)$ and depends on the cohort analysed. As an explicative example, when $\delta=10$, $\chi$ contains one element in the forecast cohort 1896 ($\ln(m_{110+,1896})$), two elements in the cohort 1897 ($\ln(m_{109,110+;1897})$), and so forth until ten elements in the last cohort 1905 ($\ln(m_{101,\ldots,110+;1896})$).

% WITHOUT LMX %%  WITHOUT LMX %%  WITHOUT LMX
% WITHOUT LMX %%  WITHOUT LMX %%  WITHOUT LMX
...
\begin{table}[h!]
	...
	\begin{tabular}{C{1.2cm}C{1.2cm}cccc|cc}
		...
		\multicolumn{1}{c}{\cellcolor{my-white}}   & \multicolumn{1}{c}{\cellcolor{my-white}}               & \multicolumn{1}{c|}{\cellcolor{my-white}$e_{40}$} & \textbf{0.05} & 0.08  & \textbf{0.05} &  0.08      \\
		\rowcolor{my-white} 
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1835-1895}}  &  \multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1896-1905}}  & 
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}10y}}& \multicolumn{1}{c|}{\cellcolor{my-white}$g_{40}$} & \textbf{0.02} &   0.11 & \textbf{0.02} &  0.09  \\
		
		%%%% SECOND EXERCISE	
		\hhline{|--------|}
		\rowcolor{my-grey} 
		\multicolumn{1}{c}{\cellcolor{my-grey}}  & \multicolumn{1}{c}{\cellcolor{my-grey}}             &
		\multicolumn{1}{c}{\cellcolor{my-grey}}  & \multicolumn{1}{c|}{\cellcolor{my-grey}$e_{40}$} & \textbf{0.04} &  0.09  & \textbf{0.05} &  0.08  \\
		...
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}15y}}               & \multicolumn{1}{c|}{\cellcolor{my-grey}$g_{40}$} & \textbf{0.02} &  0.10 & \textbf{0.03} & 0.12        \\ 
		
		%%%% THIRD EXERCISE
		\hhline{|--------|}
		\rowcolor{my-white} 
		\multicolumn{1}{c}{\cellcolor{my-white}}   &
		\multicolumn{1}{c}{\cellcolor{my-white}}   &    \multicolumn{1}{c}{\cellcolor{my-white}}                & \multicolumn{1}{c|}{\cellcolor{my-white}$e_{40}$} &  \textbf{0.03} & 0.09 & \textbf{0.04} & 0.08 \\
		\rowcolor{my-white}            
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1835-1885}}           &
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1886-1905}}               &
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}20y}}               & \multicolumn{1}{c|}{\cellcolor{my-white}$g_{40}$} & \textbf{0.04} & 0.11 & \textbf{0.04} & 0.13  \\
		
		%%%% FOURTH EXERCISE	
		\hhline{|--------|}
		\rowcolor{my-grey} 
		\multicolumn{1}{c}{\cellcolor{my-grey}}   &
		\multicolumn{1}{c}{\cellcolor{my-grey}}   & \multicolumn{1}{c}{\cellcolor{my-grey}}               & \multicolumn{1}{c|}{\cellcolor{my-grey}$e_{40}$} & \textbf{0.02} & 0.08  & \textbf{0.04} &  0.08     \\
		\rowcolor{my-grey} 
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1835-1880}}                 &  \multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1881-1905}}  & 
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}25y}}  & \multicolumn{1}{c|}{\cellcolor{my-grey}$g_{40}$} & \textbf{0.05} & 0.11 & \textbf{0.08} & 0.13      \\
		
		%%%% FIFTH EXERCISE
		\hhline{|--------|}
		\rowcolor{my-white} 
		\multicolumn{1}{c}{\cellcolor{my-white}}             &
		\multicolumn{1}{c}{\cellcolor{my-white}}             & \multicolumn{1}{c}{\cellcolor{my-white}}             & \multicolumn{1}{c|}{\cellcolor{my-white}$e_{40}$} &  \textbf{0.03} & 0.10 & \textbf{0.05} &  0.11   \\
		\rowcolor{my-white} 
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1835-1875}} &      \multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1876-1905}}               &
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}30y}}               & \multicolumn{1}{c|}{\cellcolor{my-white}$g_{40}$} & \textbf{0.06} &  0.14 & \textbf{0.10} & 0.19        \\
		
		%%%% SIXTH EXERCISE
		\hhline{|--------|}
		\rowcolor{my-grey} 
		\multicolumn{1}{c}{\cellcolor{my-grey}}   &   
		\multicolumn{1}{c}{\cellcolor{my-grey}}   &  \multicolumn{1}{c}{\cellcolor{my-grey}}                & \multicolumn{1}{c|}{\cellcolor{my-grey}$e_{40}$} & 0.10 & \textbf{0.09} & \textbf{0.08} & 0.15  \\
		\rowcolor{my-grey}           
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1835-1870}}           &
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1871-1905}}               &
		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}35y}}               & \multicolumn{1}{c|}{\cellcolor{my-grey}$g_{40}$} & \textbf{0.10} & 0.15 & \textbf{0.17} & 0.21   \\		
		
		\bottomrule 
		
	\end{tabular}
	\caption{Root mean squared errors of the C-STAD forecasts of $e_{40}$ and $g_{40}$ for adult females in Sweden (SWE) and Denmark (DNK) in six out-of-sample validation exercises: forecast horizon of 10, 15, 20, 25, 30 and 35 years.}\label{Table:RMSE}
\end{table}
...
%		\rowcolor{my-white} 
%		\multicolumn{1}{c}{\cellcolor{my-white}}   &
%		\multicolumn{1}{c}{\cellcolor{my-white}}   & \multicolumn{1}{c}{\cellcolor{my-white}}               & \multicolumn{1}{c|}{\cellcolor{my-white}$e_{40}$} & XXX    & XXX  & XXX    & XXX       \\
%		\rowcolor{my-white} 
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1835-1895}}                 &  \multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1896-1905}}  & 
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}10y}}  & \multicolumn{1}{c|}{\cellcolor{my-white}$g_{40}$} & XXX      & XXX & XXX    & XXX  \\
%		
%		%%%% SECOND EXERCISE	
%		\hhline{|--------|}
%		\rowcolor{my-grey} 
%		\multicolumn{1}{c}{\cellcolor{my-grey}}             & \multicolumn{1}{c}{\cellcolor{my-grey}}             &
%		\multicolumn{1}{c}{\cellcolor{my-grey}}             & \multicolumn{1}{c|}{\cellcolor{my-grey}$e_{40}$} & XXX   & XXX  & XXX    & XXX  \\
%		\rowcolor{my-grey}       \multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1835-1890}} &      \multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1891-1905}}               &
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}15y}}               & \multicolumn{1}{c|}{\cellcolor{my-grey}$g_{40}$} & XXX  & XXX  & XXX    & XXX         \\ 
%		
%		%%%% THIRD EXERCISE
%		\hhline{|--------|}
%		\rowcolor{my-white} 
...
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1835-1885}}           &
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1886-1905}}               &
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}20y}}               & \multicolumn{1}{c|}{\cellcolor{my-white}$g_{40}$} & XXX   & XXX & XXX    & XXX   \\
%		
%		%%%% FOURTH EXERCISE	
%		\hhline{|--------|}
%		\rowcolor{my-grey} 
%		\multicolumn{1}{c}{\cellcolor{my-grey}}   &
%		\multicolumn{1}{c}{\cellcolor{my-grey}}   & \multicolumn{1}{c}{\cellcolor{my-grey}}               & \multicolumn{1}{c|}{\cellcolor{my-grey}$e_{40}$} & XXX   & XXX   & XXX    & XXX     \\
%		\rowcolor{my-grey} 
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1835-1880}}                 &  \multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1881-1905}}  & 
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}25y}}  & \multicolumn{1}{c|}{\cellcolor{my-grey}$g_{40}$} & XXX   & XXX  & XXX    & XXX      \\
%		
%		%%%% FIFTH EXERCISE
%		\hhline{|--------|}
%		\rowcolor{my-white} 
...
%		\rowcolor{my-white} 
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1835-1875}} &      \multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}1876-1905}}               &
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-white}30y}}               & \multicolumn{1}{c|}{\cellcolor{my-white}$g_{40}$} & XXX    & XXX  & XXX    & XXX         \\
%		
%		%%%% SIXTH EXERCISE
%		\hhline{|--------|}
%		\rowcolor{my-grey} 
...
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1835-1870}}           &
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}1871-1905}}               &
%		\multicolumn{1}{c}{\multirow{-2}{*}{\cellcolor{my-grey}35y}}               & \multicolumn{1}{c|}{\cellcolor{my-grey}$g_{40}$} & XXX   & XXX & XXX    & XXX   \\		
%		
%		\bottomrule 
%		
%	\end{tabular}
...
% WITH LMX %%  WITH LMX %%  WITH LMX
% WITH LMX %%  WITH LMX %%  WITH LMX
%\begin{table}[h!]
%	\centering
%	\begin{tabular}{llccrr}
%		\toprule	
%		Fitting cohorts & Forecast cohorts & Horizon &  Measure     &  SWE   & DNK      \\ 
%		\midrule	
%		
%		%%%% FIRST EXERCISE	
%		\rowcolor{my-white} 
%		\multicolumn{1}{c}{\cellcolor{my-white}}   &
%		\multicolumn{1}{c}{\cellcolor{my-white}}   & \multicolumn{1}{c}{\cellcolor{my-white}}               & \multicolumn{1}{c}{\cellcolor{my-white}$e_{40}$} & XXX  & XXX       \\
%		\rowcolor{my-white} 
%		\multicolumn{1}{c}{\cellcolor{my-white}}   &
%		\multicolumn{1}{c}{\cellcolor{my-white}}   & \multicolumn{1}{c}{\cellcolor{my-white}}               & \multicolumn{1}{c}{\cellcolor{my-white}$G_{40}$} & XXX  & XXX       \\
%		\rowcolor{my-white} 
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-white}1835-1895}}                 &  \multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-white}1896-1905}}  & 
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-white}10y}}  & \multicolumn{1}{c}{\cellcolor{my-white}$\ln(m_{x,c})$} & XXX  & XXX   \\
%		
%		%%%% SECOND EXERCISE	
%		\hhline{|------|}
%		\rowcolor{my-grey} 
%		\multicolumn{1}{c}{\cellcolor{my-grey}}             & \multicolumn{1}{c}{\cellcolor{my-grey}}             &
%		\multicolumn{1}{c}{\cellcolor{my-grey}}             & \multicolumn{1}{c}{\cellcolor{my-grey}$e_{40}$} & XXX  & XXX   \\
%				\rowcolor{my-grey} 
%		\multicolumn{1}{c}{\cellcolor{my-grey}}             & \multicolumn{1}{c}{\cellcolor{my-grey}}             &
%		\multicolumn{1}{c}{\cellcolor{my-grey}}             & \multicolumn{1}{c}{\cellcolor{my-grey}$G_{40}$} & 	XXX  & XXX      \\
%		\rowcolor{my-grey}       \multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-grey}1835-1890}} &      \multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-grey}1891-1905}}               &
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-grey}15y}}               & \multicolumn{1}{c}{\cellcolor{my-grey}$\ln(m_{x,c})$} & XXX  & XXX        \\
%		
%		%%%% THIRD EXERCISE
%		\hhline{|------|}
%		\rowcolor{my-white} 
...
%		\multicolumn{1}{c}{\cellcolor{my-white}}   &    \multicolumn{1}{c}{\cellcolor{my-white}}                & \multicolumn{1}{c}{\cellcolor{my-white}$G_{40}$} & XXX  & XXX  \\
%		\rowcolor{my-white}            
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-white}1835-1885}}           &
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-white}1886-1905}}               &
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-white}20y}}               & \multicolumn{1}{c}{\cellcolor{my-white}$\ln(m_{x,c})$} & XXX  & XXX    \\
%		
%		%%%% FOURTH EXERCISE	
%		\hhline{|------|}
%		\rowcolor{my-grey} 
%		\multicolumn{1}{c}{\cellcolor{my-grey}}   &
%		\multicolumn{1}{c}{\cellcolor{my-grey}}   & \multicolumn{1}{c}{\cellcolor{my-grey}}               & \multicolumn{1}{c}{\cellcolor{my-grey}$e_{40}$} & 	XXX  & XXX     \\
%		\rowcolor{my-grey} 
%		\multicolumn{1}{c}{\cellcolor{my-grey}}   &
%		\multicolumn{1}{c}{\cellcolor{my-grey}}   & \multicolumn{1}{c}{\cellcolor{my-grey}}               & \multicolumn{1}{c}{\cellcolor{my-grey}$G_{40}$} & 	XXX  & XXX     \\
%		\rowcolor{my-grey} 
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-grey}1835-1880}}                 &  \multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-grey}1881-1905}}  & 
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-grey}25y}}  & \multicolumn{1}{c}{\cellcolor{my-grey}$\ln(m_{x,c})$} & XXX  & XXX      \\
%		
%		%%%% FIFTH EXERCISE
%		\hhline{|------|}
%		\rowcolor{my-white} 
...
%		\multicolumn{1}{c}{\cellcolor{my-white}}             & \multicolumn{1}{c}{\cellcolor{my-white}}             & \multicolumn{1}{c}{\cellcolor{my-white}$G_{40}$} & 	XXX  & XXX    \\		
%		\rowcolor{my-white} 
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-white}1835-1875}} &      \multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-white}1876-1905}}               &
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-white}30y}}               & \multicolumn{1}{c}{\cellcolor{my-white}$\ln(m_{x,c})$} & 	XXX  & XXX	       \\
%		
%		%%%% SIXTH EXERCISE
%		\hhline{|------|}
%		\rowcolor{my-grey} 
...
%		\multicolumn{1}{c}{\cellcolor{my-grey}}   &  \multicolumn{1}{c}{\cellcolor{my-grey}}                & \multicolumn{1}{c}{\cellcolor{my-grey}$G_{40}$} & XXX  & XXX     \\
%		\rowcolor{my-grey}           
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-grey}1835-1870}}           &
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-grey}1871-1905}}               &
%		\multicolumn{1}{c}{\multirow{-3}{*}{\cellcolor{my-grey}35y}}               & \multicolumn{1}{c}{\cellcolor{my-grey}$\ln(m_{x,c})$} & 	XXX  & XXX       \\		
%		
%		\bottomrule 
%		
%	\end{tabular}
%	\caption{Root mean squared errors of the C-STAD forecasts of $e_{40}$, $G_{40}$ and $\ln(m_{x,c})$ for adult females in Sweden (SWE) and Switzerland (CHE) in six out-of-sample validation exercises: forecast horizon of 10, 15, 20, 25, 30 and 35 years.}\label{Table:RMSE}
%\end{table}
...
\end{center}
%It is finally worth mentioning that the results are robust to the choice of the fitting period. Indeed, we performed a sensitivity analysis by modifying the fitting period in each exercise. Specifically, we fixed 1860 as the starting cohort for each exercise, and we fitted the C-STAD on the available fully observed cohorts (e.g.~1860-1895 for the first exercise, 1860-1870 for the last one). The results of this analysis are very similar to those shown in Table \ref{Table:RMSE}.
...
%Figure \ref{Fig:correlation} shows the absolute correlation of the estimated C-STAD parameters in first differences for Swedish and Swiss adult females for cohorts 1880-$\tilde{c}$, where $\tilde{c}$ is 1925 and 1924 for Sweden and Switzerland, respectively. The graph shows that the parameters of the two VAR models, $s$ and $b_U$ on one hand, and $b_L$, $c_L$ and $d_L$ on the other, are highly related to each other (light blue colours). Conversely, the correlation between the other combinations of parameters is rather low (dark blue colours).
%
%\begin{figure}[h!]
%	\begin{center}
%		\includegraphics[scale=0.58]{./Figures/FA1.pdf} 
%		\caption{Absolute correlation of the C-STAD parameters estimates (in first differences) for Swedish and Swiss adult females for cohorts 1880-$\tilde{c}$.\label{Fig:correlation}}    
%	\end{center}
%\end{figure}
