...
\begin{landscape}
	...
	\begin{table}[t]
		...
		\begin{tabular}{cccccccc|cccc}
			...
			\thead{Fitting \\ cohorts}  & \thead{Forecast \\ cohorts} & Horizon &  Measure  &  C-STAD   & \thead{2D \\ $P$-spline} & \thead{LC \\ (period)} & \thead{{\color{red}APC} \\ {\color{red}(period)}} &  C-STAD   & \thead{2D \\ $P$-spline}  & \thead{LC \\ (period)} &  \thead{{\color{red}APC} \\ {\color{red}(period)}}  \\ 
			\midrule	
			%%%% FIRST EXERCISE	
			\rowcolor{my-white} 
...
Tables \ref{Table:MAE} and \ref{Table:MAPE} show the out-of-sample results obtained using the MAE and the MAPE, respectively. The results are very similar to those obtained with the RMSE shown in Table \ref{Table:RMSE}: the C-STAD forecasts are accurate in completing the mortality experience of partially observed cohorts, with forecasts errors generally low and smaller than the 2D $P$-spline model. Moreover, both age-cohort models significantly outperform the standard LC age-period based approach, {\color{red}and they are more accurate than the APC model based on the age-period perspective}.
